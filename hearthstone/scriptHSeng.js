function countPrice()
{
    ...
    if (currency == "usd")
    {
        ...
        document.getElementById('invisiblePrice').value = price + " in dollars";
    }
    else
    if (currency == "eur")
    {
        document.getElementById('boostPrice').innerHTML = price.toFixed(0) + "&euro;";
        document.getElementById('invisiblePrice').value = price.toFixed(0) + " in Euro";
    }
    else
    if (currency == "rub")
    {
        document.getElementById('boostPrice').innerHTML = price.toFixed(0) + "&#8381;";
        document.getElementById('invisiblePrice').value = price.toFixed(0) + " in rubles";
    }
}
...
$(document).ready(function()
{
    $('#purchaseButton').click(function()
    {
        ...
        if (disRank == 0)
        {
            if (currency == "usd")
                document.getElementById('orderText3').innerHTML = "Your boost from "
                 + Number(curRank) + " to " + "LEGEND" + " on the server "
                  + document.getElementById('server').value + " costs "
                   + price + "$";
            else
            if (currency == "eur")
                document.getElementById('orderText3').innerHTML = "Your boost from "
        ...
        else
        {
            if (currency == "usd")
                document.getElementById('orderText3').innerHTML = "Your boost from "
                 + Number(curRank) + " to " + Number(disRank) + " on the server "
                  + document.getElementById('server').value + " costs "
                   + price + "$";
            else
            if (currency == "eur")
...
$(document).ready(function()
{
    $('#leftDeck').click(function()
    {
        ...
        $("#deck" + curDeck).css("display", "block");
        document.getElementById('code').innerHTML = deckCode[curDeck];
        document.getElementById('invisibleCode').value = String(deckCode[curDeck]);
    });
});
